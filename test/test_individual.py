...
class TestDisplayTree(unittest.TestCase):
    ...
    def test_display_tree_with_files(self, mock_iterdir):
        ...
        with patch("sys.stdout", new_callable=io.StringIO) as mock_stdout:
            display_tree(pathlib.Path("/test_dir"), MagicMock(s=None, a=False, f=False, d=False, t=False), "")
            output = mock_stdout.getvalue().strip()

        # Используем assert для проверки, что в выводе есть нужная информация
        assert "└── file.txt (1234 bytes)" in output, f"Expected '└── file.txt (1234 bytes)' in output, but got {output}"
    ...
    def test_display_tree_hidden_files(self, mock_iterdir):
        ...
        with patch("sys.stdout", new_callable=io.StringIO) as mock_stdout:
            display_tree(pathlib.Path("/test_dir"), MagicMock(s=None, a=True, f=True, d=False, t=False), "")
            output = mock_stdout.getvalue().strip()

        # Проверка, что скрытый файл отображается
        assert ".hidden_file (1234 bytes)" in output, f"Expected '.hidden_file (1234 bytes)' in output, but got {output}"
